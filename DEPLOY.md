...
3. For each required record in `SECURITY_BOOTSTRAP.md` (including provider
   records for Cyrene, Agnes, OpenRouter, Mistral, Cerebras and search), add the
   fresh value locally. The vault re-encrypts on save; do not put values in
   source files, forms outside the vault, or deployment manifests.

Each bot fetches its own subset on boot — the mapping is encoded in
`packages/secret-policy` and referenced by
`packages/shared/src/vault-client.ts`. Cyrene's media routes use the Agnes image
record and the shared OpenRouter record for TTS; Zoro and Shanks share the
Cerebras record. No provider key belongs in a dashboard form, Render manifest,
or repository file.
...
4. Trigger the Render deploy. The build only installs dependencies; it does
   not contact Discord or require runtime credentials, so a bad token cannot
   leave a half-built service.
5. After the service is running, register commands from a controlled operator
   environment with the matching bot credentials (for example, the bot's
   Render shell or a maintainer workstation using the documented in-memory
   credential loader): `npm run deploy:commands -w @eiflow/bot-<id>`.
   Use `--guild` only for development registration; global commands can take up
   to an hour to propagate. Treat a registration failure as a post-deploy
   credential/configuration failure, not a build failure.
6. Watch the service logs for `boot ok`. The bot is live the moment the
   gateway `ready` event fires.

## 8. (Alternative) Paid-tier note

The free Render plan may spin services down after inactivity and is not a
reliable 24/7 tier for Discord gateway connections. Expect cold starts and
possible gateway disconnects; `/health` is a liveness check, not proof that the
Discord gateway is ready. For continuous operation, use a paid Render tier, a
small VM, or Railway / Fly.io. The bot code remains host-agnostic.
...
- **Rollback** — use Render's previous successful deploy/manual deploy option
  to return to a known-good revision. Confirm `/health`, then the gateway
  `ready` log and one controlled command after rollback; command registration is
  a separate operator action and may need to be rerun if commands changed.
...
- **Rotate a secret** — generate and revoke values in the provider console,
  then seed the fresh value through the authenticated dashboard vault. For an
  HMAC rotation, add the new value to `HMAC_SECRETS_JSON` and the matching
  Render service during the overlap window. Old bots continue to work until
  their 5-minute cache expires; never record the value in docs or logs.
