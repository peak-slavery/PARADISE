...
import { createMongoTransactionStore } from './db/transaction.js';
...
import { attachGatewayReadiness, startHealthServer, type HealthDeps, type HealthServer } from './health.js';
...
import { validateCanonicalSecurityConfig } from './env.js';
...
function buildContext(
  ...
  if (!interaction.guildId) throw new Error('Guild context is required');
...
export async function createBot(options: CreateBotOptions): Promise<BotRuntime> {
  ...
  validateCanonicalSecurityConfig(bootstrapEnv);
  ...
  let serverLock: ReturnType<typeof attachServerLock> | null = null;
  const gatewayReadiness = attachGatewayReadiness(client);
  ...
  const healthDeps: HealthDeps = {
    ...
    guildLockReady: () => serverLock?.isReady() ?? false,
    gatewayReady: () => gatewayReadiness.isReady(),
  };
  ...
  const services: BotServices = {
    ...
    async executeCardTransaction(work) {
      if (!mongoHandle) throw new ServiceUnavailableError('Database');
      return createMongoTransactionStore(mongoHandle.client).executeTransaction(work);
    },
    requireSupabase() {
  ...
  /* --- server lock ------------------------------------------------------ */
  serverLock = attachServerLock(client, {
    env,
    log,
    supabase,
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    // Unsupported interaction types and DMs are rejected before any domain,
    // storage, queue, provider, or rate-limit work can run.
    if (!interaction.inGuild() || !interaction.guildId || !isApprovedGuild(interaction.guildId, env)) {
      if (interaction.isRepliable()) {
        await interaction.reply({
          content: 'This bot is restricted to its configured guild.',
    ...
    const guildId = interaction.guildId;
    if (!(await services.isAuthorized(guildId))) {
      if (interaction.isRepliable()) {
        await interaction.reply({
          content: 'This server is not enabled for this bot.',
          flags: MessageFlags.Ephemeral,
        }).catch(() => undefined);
      }
      return;
    ...
    if (interaction.isButton()) {
      ...
      const baseOperation = {
        ...
        ...logOperationFields({ guildId, requestId }),
      };
      ...
      try {
        ...
        const verdict = await enforceRateLimit(
          kv,
          keys.rateLimit(env.botId, guildId, interaction.user.id, 'button'),
        ...
        const result = await guard(
          ...
          { botId: env.botId, command: 'button', guildId, userId: interaction.user.id },
        );
        if (!result.ok) {
    ...
    try {
      ...
      // Dev-only commands are guild-scoped at registration; this runtime check
      // is the second layer in case a stale scope lingers anywhere.
      ...
      if (commandRoles && typeof commandRoles === 'object' && !Array.isArray(commandRoles)) {
