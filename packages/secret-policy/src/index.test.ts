...
describe('runtime secret policy', () => {
  it('gives every bot only common runtime records plus its own providers', () => {
    ...
    expect(getAllowedRuntimeSecrets('cyrene')).toEqual([
      ...common,
    ...
    expect(getAllowedRuntimeSecrets('cyrene')).not.toContain('provider.groq_automod.api_key');
    expect(getAllowedRuntimeSecrets('zoro')).toEqual([...common, 'provider.cerebras.api_key']);
