...
export async function POST(request: NextRequest) {
  ...
  const supabase = await createSupabaseServerClient();
  if (!supabase) return NextResponse.json({ error: 'Dashboard backend is unavailable' }, { status: 503 });
  const { data, error } = await supabase.from('infra_accounts').insert({ provider: body.provider, account_name: body.account_name, region: typeof body.region === 'string' ? body.region : null, secret_ref: body.secret_ref, endpoint: typeof body.endpoint === 'string' ? body.endpoint : null, enabled: body.enabled !== false, metadata: body.metadata && typeof body.metadata === 'object' && !Array.isArray(body.metadata) ? body.metadata : {} }).select('id,provider,account_name,region,secret_ref,endpoint,enabled,metadata,created_at,updated_at').single();
