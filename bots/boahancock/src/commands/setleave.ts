...
export const data = new SlashCommandBuilder()
  .setName('setleave')
  .setDescription('Set the channel and message sent when someone leaves')
...
export async function execute(ctx: Ctx): Promise<void> {
  ...
  const saved = await saveTemplate(ctx, 'leave', channel.id, message);
  if (!saved) throw new UserError('Could not save that setting. Please try again.');

  await ctx.replyEmbed(
    ctx.services.embeds.success('Leave message configured', `Departures will be announced in <#${channel.id}>.`, {
      fields: [
